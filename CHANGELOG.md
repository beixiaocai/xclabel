# 版本历史

## v2.6 (当前版本)
- 发布时间：2026/01/04
- **新增功能**：
  - 新增AI标注功能，支持大模型自动标注图片
  - 实现AI标注弹框，包含API配置、提示词输入和标签选择
  - 支持显示标注进度，包括已执行数量、总量、总耗时和进度条
  - 实现左侧侧边栏宽度保存功能，使用localStorage持久化
- **UI优化**：
  - 在导航栏添加AI按钮
  - 将"自动标注"按钮改为"AI配置"，URL从/auto-label改为/ai-config
  - 调整AI标注弹框宽度，使其更紧凑（从800px改为600px）
  - 优化AI标注弹框样式，使元素更紧凑，不需要上下滑动
  - 改进左侧侧边栏图片列表，解决序号和文件名重叠问题
- **Bug修复**：
  - 修复HyperLPR 404错误，使用AIAutoLabeler类统一处理API请求
  - 修复AI标注完成后标注数据清空的问题
  - 修复自定义提示弹框出现在弹框后面的问题，改为使用原生alert提示
  - 修复AI标注时未选中图片的提示问题

## v2.5
- 发布时间：2026/01/04
- **新增功能**：
  - 新增支持阿里云大模型自动标注
  - 支持保存和加载API配置参数
  - 改进了视频类型数据集的文件前缀处理，帧文件名使用视频文件名作为前缀，便于管理
- **Bug修复**：
  - 修复了自动标注功能中的'list' object has no attribute 'get'错误
  - 修复了API配置保存时的"Unexpected token '<', '<!doctype "... is not valid JSON"错误
  - 改进了JSON解析逻辑，支持处理不同格式的API返回结果
  - 增强了检测结果处理的鲁棒性，添加了类型检查确保程序稳定性

## v2.4
- 发布时间：2026/01/03
- **功能增强**：
  - 优化RTSP流处理机制，修复标注进度无返回问题
  - 改进SocketIO断开连接处理，修复disconnect handler error
  - 实现版本号统一管理，后台定义APP_VERSION变量，所有页面读取该变量显示
- **打包优化**：
  - 修复打包后运行报错问题
  - 将打包后的程序名称从app.exe改为xclabel.exe
  - 优化spec文件配置，确保所有依赖和资源正确打包
- **Bug修复**：
  - 修复自动标注功能中的各种异常处理
  - 优化异步模式配置，确保服务稳定运行
  - 修复文件管理页面无法显示文件夹和文件内容的问题
  - 修复打包程序中图片文件无法正常显示的问题
  - 改进路径处理，使用当前工作目录作为基础目录，确保程序在不同环境下正常运行

## v2.3
- 发布时间：2025/12/30
- **新增文件管理系统**：
  - 支持文件系统导航和路径浏览
  - 支持图片预览，点击图片弹框显示大图
  - 支持文件选择、全选功能
  - 支持批量下载（压缩成tar文件）和批量删除
  - 支持新建文件夹和文件上传
  - 支持文件信息查看（名称、大小、创建时间等）
  - 图片卡片显示优化，文件名自动换行不遮挡
  - 打开图片时对应卡片高亮显示，方便定位
- **UI优化**：
  - 文件管理页面与主页面风格统一
  - 优化了按钮样式和交互体验
  - 改进了对话框和模态框的样式

## v2.2
- 发布时间：2025/12/30
- **新增自动标注功能**：
  - 图片标注完成后自动显示原图和渲染图，方便对比查看
  - 图片和视频标注中的图片支持点击放大显示，提升查看体验
  - 放大显示的图片调整了顶部距离，优化显示效果
- **UI优化**：
  - 为图片添加了悬停效果，提示用户可点击放大
  - 优化了图片放大模态框的样式和动画效果
  - 支持通过ESC键关闭放大模态框，提升操作便捷性

## v2.1
- 发布时间：2025/12/01
- **新增YOLO11集成**：
  - 支持YOLO11自动安装和卸载
  - 支持YOLO11预训练模型下载和管理
  - 支持手动拖放模型文件
  - 自动检测CUDA支持
- **功能增强**：
  - 优化了标注工具，支持更流畅的标注体验
  - 改进了数据集导出功能，支持更多自定义选项
  - 新增标签管理功能，支持添加、编辑、删除标签
  - 优化了快捷键支持
- **UI优化**：
  - 重新设计了设置弹框，集成YOLO11管理功能
  - 优化了标签管理界面，支持标签颜色自定义
  - 改进了导出数据集弹框，支持文件前缀设置
  - 优化了toast提示，更加醒目易用
- **性能提升**：
  - 优化了图片加载和标注渲染性能
  - 改进了数据存储方式，提高了数据读写效率
- **Bug修复**：
  - 修复了YOLO11卸载功能，确保彻底删除所有文件
  - 优化了项目结构，移除了未使用的datasets文件夹

## v2.0
- 发布时间：2025/11/29
- **架构升级**：从Django迁移到Flask，简化了项目结构
- **功能增强**：
  - 支持视频帧提取和标注
  - 支持LabelMe数据集导入
  - 支持YOLO格式数据集导出，可自定义训练/验证/测试比例
  - 新增标签管理功能，支持添加、编辑、删除标签
  - 优化了标注工具，支持矩形、多边形等多种标注类型
- **UI优化**：
  - 重新设计了用户界面，更加简洁直观
  - 优化了标签管理界面，支持标签颜色自定义
  - 改进了导出数据集弹框，支持文件前缀设置
  - 优化了toast提示，更加醒目易用
- **性能提升**：
  - 优化了图片加载和标注渲染性能
  - 改进了数据存储方式，提高了数据读写效率

## v1.0
- 发布时间：2025/02/17
- **基础功能**：
  - 支持图像标注，包括矩形和多边形标注
  - 支持标签管理
  - 支持标注数据导出
- **技术栈**：Python + Django
- **架构**：采用传统的Django MVC架构
- v1.0 版本分支 https://gitee.com/Vanishi/xclabel/tree/v1.0
